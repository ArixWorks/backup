...
export default function AdminDomainsPage() {
  ...
  async function manageOrder(orderId: string, action: "purchased" | "complete" | "fail" | "unavailable") {
    ...
    if (action === "purchased" && !window.confirm("خرید دامنه قطعی است؟ مبلغ فریز‌شده دریافت و تاریخ خرید و انقضای یک‌ساله ثبت می‌شود.")) return
    if (action === "complete" && !window.confirm("NSهای نمایش‌داده‌شده در رجیسترار ثبت شده‌اند و سفارش تکمیل شود؟")) return
    setSaving(orderId)
    try { await apiPost("/api/v1/admin/domains", action === "fail" ? { action, orderId, reason: "ثبت دامنه انجام نشد." } : { action, orderId }); toast.success(action === "unavailable" ? "سفارش بسته، وجه آزاد و کاربر مطلع شد." : action === "purchased" ? "خرید ثبت شد؛ سفارش منتظر NS کاربر است." : "ثبت NS تأیید و سفارش تکمیل شد."); await refresh() }
  ...
  return (
    <main className="flex flex-col gap-6">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-label="نمای کلی">
        ...
        <Card><CardHeader><CardDescription>سفارش‌های باز</CardDescription><CardTitle className="text-3xl">{(value?.totals ?? []).filter((item) => ["PENDING_PURCHASE", "PROCESSING", "AWAITING_NAMESERVERS", "AWAITING_NAMESERVER_SETUP"].includes(item.status)).reduce((sum, item) => sum + item._count._all, 0).toLocaleString("fa-IR")}</CardTitle></CardHeader><CardContent><p className="text-xs text-muted-foreground">نیازمند پایش یا اقدام</p></CardContent></Card>
      ...
      <Card><CardHeader><CardTitle>گردش سفارش‌های دامنه</CardTitle><CardDescription>خرید دامنه و ثبت NS دو مرحله مستقل و قابل پیگیری هستند.</CardDescription></CardHeader><CardContent className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>شناسه</TableHead><TableHead>دامنه</TableHead><TableHead>وضعیت</TableHead><TableHead>مبلغ</TableHead><TableHead>خرید / انقضا</TableHead><TableHead>NSهای کاربر</TableHead><TableHead>عملیات</TableHead></TableRow></TableHeader><TableBody>{(value?.orders ?? []).map((order) => { const purchaseAction = ["PENDING_PURCHASE", "PROCESSING"].includes(order.status); const setupAction = order.status === "AWAITING_NAMESERVER_SETUP"; const nameservers = [order.ns1, order.ns2, order.ns3, order.ns4].filter(Boolean); return <TableRow key={order.id}><TableCell className="font-mono text-xs">{order.publicId}</TableCell><TableCell dir="ltr" className="text-left font-medium">{order.asciiDomain}</TableCell><TableCell><Badge variant="secondary">{statusLabels[order.status] ?? order.status}</Badge></TableCell><TableCell>{money(order.amountIrt)}</TableCell><TableCell className="text-xs">{order.purchasedAt ? <div className="flex min-w-max flex-col gap-1"><span>خرید: {new Date(order.purchasedAt).toLocaleDateString("fa-IR")}</span><span>انقضا: {order.expiresAt ? new Date(order.expiresAt).toLocaleDateString("fa-IR") : "—"}</span></div> : "—"}</TableCell><TableCell>{nameservers.length ? <div dir="ltr" className="flex min-w-48 flex-col gap-1 text-left font-mono text-xs">{nameservers.map((ns) => <span key={ns!}>{ns}</span>)}</div> : <span className="text-xs text-muted-foreground">هنوز ارسال نشده</span>}</TableCell><TableCell>{purchaseAction ? <div className="flex min-w-max gap-2"><Button size="sm" onClick={() => void manageOrder(order.id, "purchased")} disabled={saving !== null}>خرید انجام شد</Button><Button size="sm" variant="outline" className="border-destructive/50 text-destructive hover:bg-destructive/10" onClick={() => void manageOrder(order.id, "unavailable")} disabled={saving !== null}>قبلاً ثبت شده</Button><Button size="sm" variant="destructive" onClick={() => void manageOrder(order.id, "fail")} disabled={saving !== null}>ناموفق</Button></div> : setupAction ? <Button size="sm" onClick={() => void manageOrder(order.id, "complete")} disabled={saving !== null}>NS ثبت شد؛ تکمیل</Button> : order.status === "AWAITING_NAMESERVERS" ? <span className="text-xs text-muted-foreground">منتظر اقدام کاربر</span> : <span className="text-xs text-muted-foreground">بسته شده</span>}</TableCell></TableRow> })}</TableBody></Table></CardContent></Card>
