...
import { Archive, CheckCircle2, ChevronLeft, ChevronRight, CircleDollarSign, Download, FileSpreadsheet, Globe2, Loader2, Plus, RefreshCw, Save, Search, Upload } from "lucide-react"
...
export default function AdminDomainsPage() {
  ...
  async function archive(tld: Tld) {
    if (!window.confirm(`${tld.tld} از فروش و نتایج جست‌وجو خارج شود؟ سوابق سفارش حفظ می‌شود.`)) return
    setSaving(tld.id)
    try { await apiPost("/api/v1/admin/domains", { action: "archiveTld", id: tld.id }); toast.success(`${tld.tld} آرشیو شد.`); await refresh() }
    catch (error) { toast.error(error instanceof Error ? error.message : "آرشیو انجام نشد.") } finally { setSaving(null) }
  }

  async function manageOrder(orderId: string, action: "complete" | "fail" | "extend") {
    setSaving(orderId)
    try { await apiPost("/api/v1/admin/domains", action === "fail" ? { action, orderId, reason: "ثبت دامنه انجام نشد." } : action === "extend" ? { action, orderId, minutes: 1440 } : { action, orderId }); toast.success("وضعیت سفارش به‌روزرسانی شد."); await refresh() }
